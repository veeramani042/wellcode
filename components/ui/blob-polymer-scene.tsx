...
import { FC, useMemo, useRef, useState, useEffect, memo, useLayoutEffect } from 'react'
...
uniform vec3 u_color; // New uniform for dynamic color
varying vec2 v_uv;
...
vec3 getHolographicMaterial(vec3 normal, vec3 viewDir, float time) {
  ...
  // Use dynamic color (u_color) instead of hardcoded shades
  vec3 dynamicShades = u_color * fresnel * 1.2;
...
void main() {
  ...
  if(totalDist < tMax) {
    ...
    float spec = pow(max(dot(normal, halfDir), 0.0), 32.0); // Lower exponent for smoother specular
    
    // Get iridescent base color from dynamic color
    ...
    vec3 baseColor = vec3(0.1, 0.12, 0.15); // Dark base color
...
const ScreenPlane: FC<ScreenPlaneProps> = ({ animationState, amount }) => {
    ...
    const uniforms = useMemo(() => ({
        ...
        u_color: { value: COLOR },
    }), [viewport.width, viewport.height, animationState.positions, animationState.rotations, COLOR])
...
const Scene: FC = () => {
    ...
    return (
        <div
            className='w-full h-full bg-linear-to-b from-neutral-950 to-[#008080] absolute top-0 opacity-40'
